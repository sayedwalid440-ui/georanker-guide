...
export default function OnboardingPage() {
  ...
  // التحكم في الحقول لعمل السحر الذكي
  ...
  // دالة المراقبة الذكية (التي ستبهر المستثمر)
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // إذا كتب العميل كلمات تدل على الأثاث أو الحدائق (مثل Bahçemo)
    ...
    // إذا كتب كلمات تدل على عيادة أو أسنان
    ...
    if (detectedCategory) {
      ...
      // إزالة الاستنتاج إذا مسح العميل الكلمة
      setCategory("");
      setIsAiFilled(false);
    }
  };
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" 
              />
            </div>
            ...
            <div className="relative">
              <div className="flex justify-between items-end mb-2">
                ...
                {/* شارة الذكاء الاصطناعي التي ستظهر عند الاستنتاج */}
                {isAiFilled && (
                  <span className="text-xs font-bold text-green-400 bg-green-400/10 px-2 py-1 rounded animate-pulse">
                    {t.aiDetected}
                  </span>
                )}
              </div>
              <input 
                ...
                type="text" 
                value={category}
                onChange={(e) => { setCategory(e.target.value); setIsAiFilled(false); }}
                placeholder={t.catPlaceholder} 
                className={`w-full bg-slate-900/50 border rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition ${isAiFilled ? 'border-green-500/50 text-green-100 shadow-[0_0_15px_rgba(74,222,128,0.1)]' : 'border-white/10'}`} 
              />
            </div>
            ...
            <div>
              ...
              <input required type="text" placeholder={t.cityPlaceholder} className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" />
            </div>

            {/* Website */}
            <div>
              <label className="block text-sm font-medium mb-2 text-indigo-300">{t.website}</label>
              <input type="url" placeholder="https://..." className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" />
            </div>
            ...
            <button 
              type="submit" 
              disabled={loading} 
              className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-300 shadow-lg ${loading ? 'bg-slate-700 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-600 to-purple-600 hover:scale-[1.02] hover:shadow-indigo-500/40'}`}
            >
